...
DbPgResult::~DbPgResult() {
    if( m_res )
        m_res.reset();
}

DbResult::DataType DbPgResult::getColumnDataType(int column) const
{
    if( m_res == nullptr || m_res->size() == 0 || column >= m_res->at(0).size() )
        return DbResult::dtUnknown;
    pqxx::field fCell ( m_res->at(0).at(column) );
    return (DbResult::DataType)fCell.type();
}

int DbPgResult::getColumnNumber(const char * columnName) const
{
...
const char * DbPgResult::getColumnName(int column) const {
    if( m_res == nullptr || m_res->size() == 0 || column >= m_res->at(0).size() )
        return nullptr;
...
const char * DbPgResult::getCellData( int row, int column ) const {
...
int DbPgResult::getCellLength( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
        return -1;
...
string DbPgResult::getCell(int row, int column) const {
...
std::vector<uint8_t> DbPgResult::getCellAsByteArray( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
    ...
    const uint8_t* imBytes = static_cast<const uint8_t*>(imV);//(imBytesStr.data());
#endif
    std::vector< uint8_t > resbytes(imBytes, imBytes+nn);//= QByteArray::fromRawData( imBytes, nn);
    return resbytes;
} // Возвращает результат sql-запроса в виде QByteArray, удобно для полей типа bytea

/*
QImage DbPgResult::getCellAsImage( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
        return QImage();
...
cv::Mat DbPgResult::getCellAsMatrix( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
        return cv::Mat();
...
*/
#if PQXX_VERSION_MAJOR < 7
pqxx::binarystring DbPgResult::getCellAsBinaryString0( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
...
#else
std::basic_string<std::byte> DbPgResult::getCellAsBinaryString( int row, int column) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
...
const void* DbPgResult::getCellAsPointer( int row, int column) const {
...
bool DbPgResult::isEmpty( int row, int column ) const {
    if( m_res == nullptr || row >= m_res->size() || column >= m_res->at(row).size() )
        return true;
...
int DbPgResult::getRowCount() const {
    if( m_res == nullptr )
        return -1;
    return m_res->size();
}

int DbPgResult::getColumnCount() const {
...
DbResult::ResultStatus DbPgResult::resultStatus() const {
    if( m_res == nullptr )
        return DbResult::Unknown;

    return DbResult::CommandOk;
}

string DbPgResult::errorMessage() const {
    return std::string();
}

string DbPgResult::errorCode() const {
    return std::string();
}

void DbPgResult::next() {
}
