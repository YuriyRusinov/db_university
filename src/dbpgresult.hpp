...
public:
    DbPgResult(std::shared_ptr<pqxx::result> res) : DbResult(), m_res(res) {};
    DbPgResult( const DbPgResult& dbr ) = delete;
    DbPgResult& operator=( const DbPgResult& dbr ) = delete;
    virtual ~DbPgResult();

    virtual DbResult::DataType getColumnDataType(int column) const override;
    ...
    virtual DbResult::ResultStatus resultStatus() const override;
...
private:
    ...
    std::shared_ptr<pqxx::result> m_res;
    int rowCount;
    int columnCount;
