...
class DbResult {
public:
    DbResult() : currentRow(-1) {};
    DbResult( const DbResult& dbr ) = delete;
    DbResult& operator=( const DbResult& dbr ) = delete;
    virtual ~DbResult() {};
    enum ResultStatus{
        EmptyQuery = 0,
        CommandOk = 1,
    ...
    std::string getColumnNameAsString(int column) const;
    ...
    virtual std::string getCell(int row, int column) const = 0; // Возвращает результат sql-запроса в формате QVariant
    virtual std::vector<uint8_t> getCellAsByteArray (int row, int column) const = 0; // Возвращает результат sql-запроса в виде QByteArray, удобно для полей типа bytea
    ...
    inline virtual std::string getCell(int column) const
    {
    ...
    inline std::string getCellAsString(int column, bool * ok = 0) const
    {
    ...
    inline std::vector< std::string > getCellAsStringList(int column, bool * ok = 0) const
    {
        return getCellAsStringList(currentRow, column, ok);
    }
    inline std::string getCellAsDateTime(int column, bool * ok = 0) const
    {
